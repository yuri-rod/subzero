...
def test_parse_fps_reads_fractions_and_plain_values():
    from subzero.worker.jellyfin import parse_fps
...
def test_media_carries_the_video_frame_rate(tmp_path):
    video = tmp_path / "Filme.mkv"
    video.write_bytes(b"x")
    streams = [dict(PLAYBACK["MediaSources"][0]["MediaStreams"][0],
                    AverageFrameRate="24000/1001")]
    payload = {"MediaSources": [dict(PLAYBACK["MediaSources"][0], Path=str(video),
                                     MediaStreams=streams)]}
    c = client({("GET", "http://jf/Items/abc/PlaybackInfo"): (200, payload),
                ("GET", "http://jf/Items"): (200, {"Items": []})})

    assert c.media("abc").fps == 24000 / 1001
